...
class PaymentOrderEntityTest {
    ...
    void testCreatePaymentOrderWithStaticMethod() {
        // When
        PaymentOrderEntity newPaymentOrder = PaymentOrderEntity.create(
                EXTERNAL_REFERENCE,
                DEBTOR_ACCOUNT,
                CREDITOR_ACCOUNT,
                AMOUNT,
    ...
    void testUpdateWithToBuilder() {
        ...
        // When
        PaymentOrderEntity updatedPaymentOrder = paymentOrder.toBuilder()
                .status(newStatus)
                .amount(newAmount)
    ...
    void testEqualsAndHashCode() {
        // Given
        PaymentOrderEntity samePaymentOrder = PaymentOrderEntity.builder()
                .id(ID)  // Mismo ID
        ...
        PaymentOrderEntity differentPaymentOrder = PaymentOrderEntity.builder()
    ...
    void testNullValues() {
        // Given
        PaymentOrderEntity nullPaymentOrder = PaymentOrderEntity.builder()
                .id(null)
    ...
    void testCreateWithZeroAmount() {
        // When
        PaymentOrderEntity zeroAmountOrder = PaymentOrderEntity.create(
                EXTERNAL_REFERENCE,
                DEBTOR_ACCOUNT,
                CREDITOR_ACCOUNT,
                BigDecimal.ZERO,
    ...
    void testDifferentStatuses() {
        ...
        for (PaymentOrderStatusEnum status : statuses) {
            // When
            PaymentOrderEntity statusOrder = paymentOrder.toBuilder()
                    .status(status)
                    .build();
    ...
    void testCreationAndUpdateDateOnCreation() {
        // When
        PaymentOrderEntity newOrder = PaymentOrderEntity.create(
                "NEW-REF",
                "DEBTOR1",
                "CREDITOR1",
    ...
    void testEquals_AllFields() {
        LocalDateTime now = LocalDateTime.now();
        PaymentOrderEntity po1 = PaymentOrderEntity.builder()
                .id("1")
        ...
        PaymentOrderEntity po2 = PaymentOrderEntity.builder()
                .id("1")
                .externalReference("EXT")
                .status(PaymentOrderStatusEnum.PENDING)
                .debtorAccount("A")
    ...
    void testEquals_Different() {
        PaymentOrderEntity po1 = PaymentOrderEntity.builder().id("1").build();
        PaymentOrderEntity po2 = PaymentOrderEntity.builder().id("2").build();
